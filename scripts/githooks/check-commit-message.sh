...
function check_jira_ref {
  ...
  if [[ "$COMMIT_MESSAGE" =~ ^(feat|fix|chore|docs|style|refactor|perf|test|ci|build|revert|style)(\([a-z0-9_-]+\))?:[[:space:]] ]] &&
    [[ "$JIRA_REF" =~ ^(FTRS|FDOS|DOSIS|SIA)-[0-9]+$ ]] &&
    [[ "$COMMIT_MESSAGE" != *"$JIRA_REF"* ]]; then
    COMMIT_MESSAGE=$(echo "$COMMIT_MESSAGE" | sed -E "s/^((feat|fix|chore|docs|style|refactor|perf|test|ci|build|revert|style)(\([a-z0-9_-]+\))?:)/\1 $JIRA_REF/")
  fi
...
function check_commit_message_format {
  COMMIT_MESSAGE="$1"
  local REGEX='^(feat|fix|chore|docs|style|refactor|perf|test|ci|build|revert|style)(\([a-z0-9_-]+\))?: (FTRS|FDOS|DOSIS|SIA)-[0-9]+ .+'
