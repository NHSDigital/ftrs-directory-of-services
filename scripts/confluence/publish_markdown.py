...
def wrap_markdown_macro(markdown_text: str) -> str:
    # Storage format wrapping a Markdown macro with CDATA to preserve content
    return (
...
def publish_one(base_url: str, auth_mode: str, user: Optional[str], token: str, space: str, title: str, md_text: str, parent_id: Optional[str], storage_override: Optional[str] = None) -> str:
    headers = bearer_auth_header(token) if auth_mode == "bearer" else basic_auth_header(user or "", token)
    storage = storage_override if storage_override is not None else wrap_markdown_macro(md_text)
    existing = None
    if parent_id:
...
def main() -> None:
    ...
    for f in args.files:
        ...
        try:
            # Inject Children Display macro for service index pages so links to child pages work
            storage_override = None
            is_service_index = ('/nfr-by-service/' in pstr and path.name.lower() == 'index.md')
            if is_service_index:
                storage_override = wrap_markdown_macro(md) + children_display_macro()
            page_id = publish_one(base_url, args.auth, args.user, args.token, args.space, title, md, parent_for_file, storage_override=storage_override)
            ...
            if args.allow_create_if_missing and parent_for_file:
                ...
                if is_service_index:
                    storage_override = wrap_markdown_macro(md) + children_display_macro()
                body = storage_override if storage_override is not None else wrap_markdown_macro(md)
