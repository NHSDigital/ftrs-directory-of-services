...
stage: set-prerelease-version
	...
	aws s3 cp s3://$(ARTEFACT_DEVELOPMENT_PATH)/ s3://$(ARTEFACT_STAGING_PATH)/ --recursive --metadata-directive COPY --region $(AWS_REGION)
...
release-candidate: set-prerelease-version
	...
	aws s3 cp s3://$(ARTEFACT_STAGING_PATH)/ s3://$(ARTEFACT_RELEASE_CANDIDATE_PATH)/ --recursive --metadata-directive COPY --region $(AWS_REGION)
...
release:
	...
	aws s3 cp s3://$(ARTEFACT_RELEASE_CANDIDATE_PATH)/ s3://$(ARTEFACT_RELEASE_PATH)/ --recursive --metadata-directive COPY --region $(AWS_REGION)
