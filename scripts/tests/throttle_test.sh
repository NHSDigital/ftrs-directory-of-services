...
# NOTE: These are assumptions used only for prediction (expected first 429)
# and for --dry-run simulation. They do NOT modify server throttling and do NOT
# change the client send pattern beyond -r/-d.
THEORY_RATE=3
THEORY_BURST=9
...
Options:
  ...
  --theory-rate N         Assumed server rate limit for prediction/simulation only (default ${THEORY_RATE})
  --theory-burst N        Assumed server burst limit for prediction/simulation only (default ${THEORY_BURST})
...
Notes:
  ...
  - --theory-rate/--theory-burst do not modify API Gateway throttling and do not shape client traffic; they are used
    to compute the expected time to first 429 and to simulate outcomes when --dry-run is enabled
EOF
}
...
PY
    ...
    if [[ $VERBOSE -eq 1 ]]; then
      ...
      # Capture high-precision finish timestamp if python3 is available
      if [[ "$USE_FLOAT" -eq 1 ]]; then
        finish_ts=$(python3 - <<'PY'
import time
print(f"{time.time():.6f}")
PY
)
      ...
      printf "i=%05d t=%4ds sched=%.3fs finish_ts=%s status=%s t_total=%ss\n" "$i" "$rel" "$offset" "$finish_ts" "$status" "$ttime"
    fi
  ) &
done
...
# Compute first 429 by minimum scheduled index (not first completion line)
FIRST_429_IDX=$(awk '($2=="429"){ if(min=="" || ($1+0) < min) min=$1 } END{ if(min!="") print min }' "$RAW_LOG" || true)
if [[ -n "$FIRST_429_IDX" ]]; then FIRST_429_SEC=$(awk -v idx="$FIRST_429_IDX" -v r="$RATE" 'BEGIN { printf "%.3f", idx/r }'); else FIRST_429_SEC=""; fi
# Also estimate finish time for the first 429: schedule_time + time_total for that index
FIRST_429_TTOTAL=""; if [[ -n "$FIRST_429_IDX" ]]; then FIRST_429_TTOTAL=$(awk -v idx="$FIRST_429_IDX" '($1==idx && $2=="429"){ print $3; exit }' "$RAW_LOG" || true); fi
FIRST_429_FINISH_EST=""; if [[ -n "$FIRST_429_SEC" && -n "$FIRST_429_TTOTAL" ]]; then FIRST_429_FINISH_EST=$(awk -v s="$FIRST_429_SEC" -v tt="$FIRST_429_TTOTAL" 'BEGIN { printf "%.3f", s + tt }'); fi
...
# Estimated starting tokens from observed first 429 (only meaningful if RATE>Theory_Rate and FIRST_429_SEC set)
EST_START_TOKENS=""; if [[ -n "$FIRST_429_SEC" ]]; then EST_START_TOKENS=$(awk -v r="$RATE" -v tr="$THEORY_RATE" -v t="$FIRST_429_SEC" 'BEGIN { if (r>tr) printf "%.0f", (r-tr)*t; }'); fi
...
if [[ -n "$FIRST_429_FINISH_EST" ]]; then echo "  First 429 (finish est): ~${FIRST_429_FINISH_EST}s"; fi
if [[ -n "$EXPECTED_FIRST" ]]; then echo "  Expected first 429 ~${EXPECTED_FIRST}s (rate=$THEORY_RATE, burst=$THEORY_BURST, rps=$RATE)"; fi
if [[ -n "$EST_START_TOKENS" ]]; then echo "  Est. starting tokens: ~$EST_START_TOKENS (from observed first 429)"; fi
echo "  Warmup applied:   ${WARMUP}s"
echo "  Status counts:"; echo "$COUNTS" | awk '{printf "    %s: %s\n", $1, $2}'
# New: show scheduled-per-second counts based on index/RATE
echo "  Scheduled per-second counts (by index/RATE):"
awk -v rate="$RATE" '{ s = int($1 / rate); c[s]++ } END { for (s in c) printf "%6ds: %d\n", s, c[s] }' "$RAW_LOG" | sort -n
...
if [[ $AS_JSON -eq 1 ]]; then
  ...
  if [[ -n "$FIRST_429_FINISH_EST" ]]; then echo "  \"first_429_finish_est_seconds\": $FIRST_429_FINISH_EST,"; else echo "  \"first_429_finish_est_seconds\": null,"; fi
  if [[ -n "$EST_START_TOKENS" ]]; then echo "  \"est_starting_tokens\": $EST_START_TOKENS,"; else echo "  \"est_starting_tokens\": null,"; fi
