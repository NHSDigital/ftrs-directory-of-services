...
# Resolve scripts relative to this wrapper's location so it works when invoked from any CWD
...
# Run the token command and capture stdout (status messages should go to stderr from the token script)
