...
# This script runs service automation APIM tests
# It retrieves the necessary tokens from Proxygen and runs tests with APIM parameters
...
if [ -z "$APPLICATION_TEST_DIR" ]; then
  echo "Set APPLICATION_TEST_DIR to directory holding test code"
  EXPORTS_SET=1
fi
...
# Run tests with APIM markers, passing api-name and proxy-name
poetry run pytest \
  -m "is-apim" \
  --api-name="$API_NAME" \
...
if [ $TEST_RESULTS -ne 0 ]; then
  echo "APIM service automation tests have failed"
  exit $TEST_RESULTS
else
  echo "APIM service automation tests have passed"
  exit 0
fi
