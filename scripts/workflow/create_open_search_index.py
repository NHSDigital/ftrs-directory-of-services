...
def sign_request_and_send(method: str, url: str, body: str, region: Optional[str], service: str = "aoss") -> requests.Response:
...
def sign_request_and_put(url: str, body: str, region: Optional[str], service: str = "aoss") -> requests.Response:
    return sign_request_and_send("PUT", url, body, region, service)


def sign_request_and_delete(url: str, region: Optional[str], service: str = "aoss") -> requests.Response:
    return sign_request_and_send("DELETE", url, "", region, service)


def sign_request_and_head(url: str, region: Optional[str], service: str = "aoss") -> requests.Response:
    return sign_request_and_send("HEAD", url, "", region, service)
...
class IndexCreator:
    ...
    def create_index(self) -> int:
        ...
        check_url = url
        try:
            log.info('Checking if index exists: %s', check_url)
            head_resp = sign_request_and_head(check_url, self.aws_region, 'aoss')
        except requests.RequestException as exc:
            log.error('Failed to check index existence %s: %s', final_index, exc)
            return 4

        if head_resp.status_code == 200:
            try:
                log.info('Index exists, deleting: %s', url)
                del_resp = sign_request_and_delete(url, self.aws_region, 'aoss')
                if del_resp.status_code in (200, 202, 204):
                    log.info('Deleted existing index %s', final_index)
                else:
                    log.error('Failed to delete index %s: %s %s', final_index, del_resp.status_code, del_resp.text)
                    return 4
            except requests.RequestException as exc:
                log.error('Failed to delete existing index %s: %s', final_index, exc)
                return 4
        elif head_resp.status_code == 404:
            log.info('Index %s does not exist; will create', final_index)
        else:
            log.error('Unexpected response when checking index %s: %s %s', final_index, head_resp.status_code, head_resp.text)
            return 4
