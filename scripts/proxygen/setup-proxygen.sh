...
#
# Description: Sets up the Proxygen CLI locally to enable local use of the Proxygen CLI
#
# Prerequisites:
#   - asdf installed
#   - yq installed
...
touch "$SETTINGS_FILE"
PROXYGEN_API_NAME=$PROXYGEN_API_NAME PROXYGEN_URL=$PROXYGEN_URL \
  yq eval '
    .api = env(PROXYGEN_API_NAME) |
    .endpoint_url = env(PROXYGEN_URL) |
