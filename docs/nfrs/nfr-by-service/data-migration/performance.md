...
|--------|------|-------------|-------------|---------|
| Performance | [PERF-001](#DataMigration–PerformanceNFRs&Operations-PERF-001) | Each operation meets registry-defined percentile targets (p50/p95) logged & asserted (see performance/expectations.yaml) | Each API or batch operation meets agreed median and 95th percentile latency targets. | (none) |
| Performance | [PERF-006](#DataMigration–PerformanceNFRs&Operations-PERF-006) | Batch window p95 latency delta ≤5% | Batch window latency stays within a small variance (e.g., p95 delta ≤ defined %). | (none) |
