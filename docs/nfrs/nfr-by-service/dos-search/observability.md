# FtRS NFR – Service: DoS Search – Domain: Observability
...
| Observability | OBS-005 | Performance metrics per layer present | Layered performance metrics (app, DB, cache) are visible. | [FTRS-1016 Draft set of performance metrics](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1016) |
...
| Observability | [OBS-008](#obs-008) | TPS per endpoint displayed & threshold alert configured | Per-endpoint transactions per second (TPS) are displayed with alert thresholds. | [FTRS-1688 Alerting for the Search by ODS code endpoint](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1688) |
...
| Observability | [OBS-011](#obs-011) | Failure types logged & classified in dashboard | Failure types are logged and classified for reporting. | (none) |
| Observability | [OBS-012](#obs-012) | Error percentage metric & alert configured | Error rate metric and alert exist to highlight reliability issues. | [FTRS-1017 Discover and Document Real-Time Alerting Strategy and Escalation Paths](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1017) |
| Observability | [OBS-013](#obs-013) | Infra log query returns expected fields | Infrastructure logs return expected structured fields for queries. | [FTRS-323 Platform Logging & Monitoring](https://nhsd-jira.digital.nhs.uk/browse/FTRS-323) |
...
| Observability | OBS-018 | Log level propagation <2min with alert on breach | Log level changes propagate quickly (under defined minutes) with alert if breach. | [FTRS-967 Incorrect Log Level in the log base for Data Fetch Operations](https://nhsd-jira.digital.nhs.uk/browse/FTRS-967) |
...
| Observability | [OBS-030](#obs-030) | Distributed trace spans cover end-to-end request | Distributed tracing spans cover end-to-end request path. | [FTRS-885 Update EPS logs to include Tracing ID for end to end audit logging](https://nhsd-jira.digital.nhs.uk/browse/FTRS-885) |
...
| Observability | OBS-032 | Per-endpoint 4xx/5xx response metrics & alert thresholds configured | Per-endpoint 4xx and 5xx response metrics are captured with alert thresholds so error rate spikes are detected and acted upon quickly. | [FTRS-1573 Http status class metrics](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1573) |
...
|----------|-------|---------|-------|----|--------|------|-------|---------|
| health-panels-green | App & infra health panels show green | All critical panels green; no stale data | Continuous + CI verification on change | int,ref,prod | DoS Search | draft | [FTRS-1015 Enable and Set Up Monitoring Endpoints](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1015) | Ensures at-a-glance service health visibility |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| perf-metrics-latency | Performance metrics latency ≤60s | Metrics pipeline delivers data within 60s latency | Continuous monitoring | int,ref,prod | DoS Search | draft | (none) | Fresh metrics are required for accurate operational decisions |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| tps-threshold-alert | TPS per endpoint displayed & threshold alert configured | TPS dashboard present; alert rule configured and tested | CI validation + monthly alert fire drill | int,ref,prod | DoS Search | draft | [FTRS-1688 Alerting for the Search by ODS code endpoint](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1688) | Detects throughput anomalies proactively |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| latency-histograms | Endpoint latency histograms with p50/p95/p99 | Histograms available per endpoint with p50/p95/p99 series | Continuous | int,ref,prod | DoS Search | draft | (none) | Percentile visibility supports performance governance |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| aggregate-latency-accuracy | Aggregate latency panel accurate within 2% roll-up | Roll-up accuracy within \u22642% vs raw series | Monthly calibration | prod | DoS Search | draft | (none) | Ensures trustworthy aggregate metrics |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| failure-type-classification | Failure types logged & classified in dashboard | 100% failures carry type; classification accuracy \u2265 95% | Continuous + monthly accuracy audit | int,ref,prod | DoS Search | draft | (none) | Improves incident triage |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| error-percentage-alert | Error percentage metric & alert configured | Alert triggers when error% > 1% over 5m; playbook linked | Continuous + monthly tuning | prod | DoS Search | draft | [FTRS-1017 Discover and Document Real-Time Alerting Strategy and Escalation Paths](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1017) | Early detection of reliability regressions |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| infra-log-query-fields | Infra log query returns expected fields | Queries return required fields (timestamp, severity, host, correlation_id) | CI per build + weekly audit | int,ref,prod | DoS Search | draft | [FTRS-323 Platform Logging & Monitoring](https://nhsd-jira.digital.nhs.uk/browse/FTRS-323) | Ensures log usability for ops |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| infra-log-required-fields | Infra log entries include required fields | 100% entries include required fields; schema lint passes | CI per build + monthly audit | int,ref,prod | DoS Search | draft | (none) | Guarantees structured logging quality |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| distributed-trace-coverage | Distributed trace spans cover end-to-end request | ≥95% of requests include spans across key tiers | Continuous + monthly sampling review | int,ref,prod | DoS Search | draft | [FTRS-885 Update EPS logs to include Tracing ID for end to end audit logging](https://nhsd-jira.digital.nhs.uk/browse/FTRS-885) | Enables end-to-end diagnosis and correlation across layers |
...
| Control ID | Measure | Threshold | Cadence | Envs | Services | Status | Stories | Rationale |
|----------|-------|---------|-------|----|--------|------|-------|---------|
| unauth-access-monitoring | Unauthorized API access attempts logged & alerted with context | 100% auth failures & forbidden requests produce structured log entry with reason, correlation_id, source_ip, user_agent; alert triggers on >5 failed auth attempts per principal per 1m or anomaly spike (>3x baseline) | Continuous collection + weekly anomaly review + monthly rule tuning | int,ref,prod | DoS Search | draft | [FTRS-1607 Unauthorised API access monitoring](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1607), [FTRS-1650 Adjust API Gateway Rate limiting to cater for 150tps](https://nhsd-jira.digital.nhs.uk/browse/FTRS-1650) | Early detection of credential stuffing, token misuse, and privilege escalation attempts |
