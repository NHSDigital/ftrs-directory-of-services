...
jira_key:
title: gp-search gp-lookup-ods Performance
role: API Consumer
goal: Receive response from gp-lookup-ods within performance targets
...
## Description
Implement and validate performance for gp-search operation: gp-lookup-ods.
...
- Service: gp-search
- Path: /gp-search/ods/{code}
- Method: GET
- Concurrency profile: burst-60 steady-15
- Status: draft

## Monitoring & Metrics
- `gp_lookup_ods_latency_ms` histogram (p50, p95, p99)
- `gp_lookup_ods_requests_total` counter
- `gp_lookup_ods_errors_total` counter
- `gp_lookup_ods_tps` gauge
