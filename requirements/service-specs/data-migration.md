...
## 11. Observability
Logging: Powertools Logger with structured events (DM_ETL_* codes). Keys: `run_id`, `env`, `workspace`, `record_id`, transformer name, elapsed_time, counts. Metrics: aggregated via final DM_ETL_999 log snapshot (`migrated_records`, `errors`). Tracing: correlation IDs appended; can extend to X-Ray/OTel spans later. Alert rules (future): error rate >1% or duration >threshold.
