...
| Fetch FtRS UUID         | `pipeline/extract.fetch_organisation_uuid(ods_code)`          | Queries internal `/Organization?identifier=https://fhir.nhs.uk/Id/ods-organization-code                | {code}` Bundle to locate existing UUID | UUID or None |
