...
| fix | fix(requirements): Correct p95 target for gp-list <JIRA-ID> |
| docs | docs(requirements): Add explanation for PERF-014 <JIRA-ID> |
| chore | chore(scripts): Refactor NFR page generator <JIRA-ID> |
| refactor | refactor(requirements): Consolidate duplicated README sections <JIRA-ID> |
...
4. Validate: `python3 scripts/jira/validate_artifacts.py` (checks front matter, mandatory spec sections, criteria count, NFR codes).
5. Export paste-ready text: `python3 scripts/jira/export_stories.py` → `.jira-export/*.txt` (copy into Jira when available).
...
| Export | `scripts/jira/export_stories.py` | Generates Jira paste text |
