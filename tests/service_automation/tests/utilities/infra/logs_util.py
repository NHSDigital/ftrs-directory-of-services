...
def get_log_group_name(lambda_name: str) -> str:
    ...
    return f"/aws/lambda/{project}-{environment}-{lambda_name}{workspace_suffix}"
...
def get_logs(
    ...
    Args:
        ...
        retry_count (int, optional): Maximum number of polling retries. Defaults to 32.
    ...
    while not logs_found:
        try:
            ...
            if error.response["Error"]["Code"] == "LimitExceededException":
                print("Limit exceeded, retrying after 30 seconds...")
                sleep(30)
                continue
            raise
        ...
        if response["results"]:
            ...
            msg = f"Log search retries exceeded.. no logs found for query: {query}"
            print(msg)
