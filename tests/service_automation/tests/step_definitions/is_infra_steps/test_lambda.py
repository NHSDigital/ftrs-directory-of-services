...
@when(('I invoke the lambda with an empty ods code'), target_fixture='fLambda_payload')
def invoke_lambda(aws_lambda_client, flambda_name):
    lambda_params = {
        "odsCode": ""
    }
    lambda_payload = aws_lambda_client.invoke_function(flambda_name, lambda_params)
...
def lambda_error_message(fLambda_payload, error_message, status_code):
    ...
    logger.debug("Checking for error message {}", response["issue"][0]["details"]["coding"][0]["display"])
