...
def get_repo_fixture(request: pytest.FixtureRequest, repo_name: str) -> AttributeLevelRepository:
    repo_fixture_name = f"{repo_name}_repo"
    repo: AttributeLevelRepository = request.getfixturevalue(repo_fixture_name)
    return repo

@given(parsers.parse('I have a {repo_name} repo that is seeded'), target_fixture="model_repo")
def get_repo_fixture(request: pytest.FixtureRequest, repo_name: str) -> AttributeLevelRepository:
...
@when(parsers.parse('I get a model with id "{model_id}" from the repo'), target_fixture="get_model_result")
def get_from_repo(model_repo: AttributeLevelRepository, model_id: str) -> ModelType | None:
    return model_repo.get(model_id)


@when(parsers.parse('I get a model with ODS code "{ods_code}" from the repo'), target_fixture="get_model_result")
def get_from_repo(model_repo: AttributeLevelRepository, ods_code: str) -> ModelType | None:
