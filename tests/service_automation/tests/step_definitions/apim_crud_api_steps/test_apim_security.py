...
def step_put_no_auth_org(dos_ingestion_service_url: str, endpoint: str):
    payload = _load_default_payload()
    org_id = payload.get("id")
    full_endpoint = build_endpoint(endpoint, org_id)
    payload = payload.copy()
    return send_api_request(
        "PUT", full_endpoint, dos_ingestion_service_url, api_key=None, payload=payload
    )
...
def step_put_invalid_key_org(
    dos_ingestion_service_url: str, endpoint: str, api_key: str
):
    payload = _load_default_payload()
    org_id = payload.get("id")
    full_endpoint = build_endpoint(endpoint, org_id)
...
def step_validate_status(fresponse, status_code: int):
    logger.info(
        f"Validating status code: expected={status_code}, actual={fresponse.status_code}"
    )
    assert fresponse.status_code == status_code, (
        f"Expected {status_code}, got {fresponse.status_code}"
    )
