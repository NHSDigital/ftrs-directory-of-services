...
Feature: Service Migration - Validation Failures
  ...
  Scenario Outline: Services with missing required fields are rejected
    ...
    Then the SQS event metrics should be 1 total, <expected_supported> supported, <expected_unsupported> unsupported, <expected_transformed> transformed, 0 migrated, 0 skipped and <expected_errors> errors
  ...
  Scenario Outline: Services with invalid ODS code format are rejected
    Given a 'Service' exists called '<service_name>' in DoS with attributes:
      ...
      | restricttoreferrals | false                        |
    When the data migration process is run for table 'services', ID '<service_id>' and method 'insert'
    Then the SQS event metrics should be 1 total, 0 supported, 1 unsupported, 0 transformed, 0 migrated, 0 skipped and 0 errors
    And no organisation was created for service '<service_id>'
    And no location was created for service '<service_id>'
    And no healthcare service was created for service '<service_id>'
  ...
  Scenario: Unsupported operation method is not processed
    ...
    Then the SQS event metrics should be 0 total, 0 supported, 0 unsupported, 0 transformed, 0 migrated, 0 skipped and 0 errors
  ...
  Scenario Outline: Services with inactive status are not included
    Given a 'Service' exists called '<service_name>' in DoS with attributes:
      ...
      | restricttoreferrals | false                        |
    When the data migration process is run for table 'services', ID '<service_id>' and method 'insert'
    Then the SQS event metrics should be 1 total, 1 supported, 0 unsupported, 0 transformed, 0 migrated, 1 skipped and 0 errors
    And no organisation was created for service '<service_id>'
    And no location was created for service '<service_id>'
    And no healthcare service was created for service '<service_id>'
