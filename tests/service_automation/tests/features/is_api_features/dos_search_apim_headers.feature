...
Feature: API DoS Service validates headers
  ...
  Scenario: I cannot search APIM for GP Endpoint with an invalid version
    ...
    And the OperationOutcome contains an issue with severity "error"
    And the OperationOutcome contains an issue with code "value"
    And the OperationOutcome contains an issue with diagnostics "Invalid version found in supplied headers: version - 2"
    And the OperationOutcome contains an issue with details for REC_BAD_REQUEST_CODING coding

  Scenario: I cannot search APIM for GP Endpoint with an invalid header
    ...
    And the OperationOutcome contains an issue with severity "error"
    And the OperationOutcome contains an issue with code "value"
    And the OperationOutcome contains an issue with diagnostics "Invalid request headers supplied: evil-header"
    And the OperationOutcome contains an issue with details for REC_BAD_REQUEST_CODING coding
