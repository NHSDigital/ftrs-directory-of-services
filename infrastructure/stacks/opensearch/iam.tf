    ...
    Statement = [
      {
        ...
        Resource = flatten([
          ...
          [
            "arn:aws:dynamodb:${var.aws_region}:${local.account_id}:table/${local.project_prefix}-database-${tablename}${local.workspace_suffix}",
            "arn:aws:dynamodb:${var.aws_region}:${local.account_id}:table/${local.project_prefix}-database-${tablename}${local.workspace_suffix}/export/*",
            "arn:aws:dynamodb:${var.aws_region}:${local.account_id}:table/${local.project_prefix}-database-${tablename}${local.workspace_suffix}/stream/*"
          ]
        ])
      },
      {
