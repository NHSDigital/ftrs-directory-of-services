resource "aws_osis_pipeline" "dynamodb_to_opensearch_osis_pipeline" {
  for_each = local.stack_enabled == 1 ? toset(var.dynamodb_table_names_for_opensearch) : toset([])
  ...
  log_publishing_options {
    cloudwatch_log_destination {
      log_group = aws_cloudwatch_log_group.osis_pipeline_cloudwatch_log_group[0].name
    }
  ...
  pipeline_configuration_body = templatefile("pipeline-config.yaml.tmpl", {
    ...
    opensearch_endpoint  = module.opensearch_serverless[0].endpoint
    index_name           = "${each.value}${local.workspace_suffix}"
    s3_bucket            = module.s3[0].s3_bucket_id
    s3_prefix            = var.ddb_export_s3_prefix
    network_policy_name  = try("pipeline-${aws_opensearchserverless_security_policy.opensearch_serverless_network_access_policy[0].name}", "pipeline-${data.aws_opensearchserverless_security_policy.opensearch_serverless_network_access_policy[0].name}")
    max_sink_retries     = var.max_sink_retries
    s3_dlq_bucket        = module.s3_opensearch_pipeline_dlq_bucket[0].s3_bucket_id
