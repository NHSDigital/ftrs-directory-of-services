...
data "aws_iam_policy_document" "osis_apigw_log_group_policy_document" {
  ...
  statement {
    ...
    actions = ["logs:CreateLogStream", "logs:PutLogEvents"]
    resources = ["arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/vendedlogs/OpenSearchIngestion/*:log-stream:*",
    "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/apigateway/*/default:log-stream:*"]
    condition {
      test     = "ArnLike"
