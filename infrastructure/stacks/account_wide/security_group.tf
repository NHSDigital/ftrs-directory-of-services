...
# Security Group with Egress rules to Internet and DynamoDB VPCE
resource "aws_security_group" "internet_dynamodb_egress_security_group" {
...
resource "aws_vpc_security_group_egress_rule" "allow_dynamodb_access_rule" {
  security_group_id = aws_security_group.internet_dynamodb_egress_security_group.id
  description       = "Egress rule to allow DynamoDB traffic on 443"
...
# trivy:ignore:aws-vpc-no-public-egress-sgr : TODO https://nhsd-jira.digital.nhs.uk/browse/FTRS-386
resource "aws_vpc_security_group_egress_rule" "allow_443_rule" {
  security_group_id = aws_security_group.internet_dynamodb_egress_security_group.id
  description       = "Egress rule to allow internet traffic on 443"
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "tcp"
  to_port           = var.https_port
  from_port         = var.https_port
