...
data "aws_iam_policy_document" "ods_etl_scheduler_invoke_policy" {
  ...
  statement {
    ...
    resources = [
      data.aws_kms_key.scheduler_kms_key.arn
    ]
  }
}
...
data "aws_kinesis_firehose_delivery_stream" "raw_firehose_stream" {
  name = "${local.project_prefix}-${var.firehose_stack}-${var.raw_firehose_name}"
}
