...
resource "aws_cloudwatch_metric_alarm" "route53_health_check_status_critical" {
...
resource "aws_cloudwatch_metric_alarm" "route53_health_check_percentage_healthy_critical" {
  count    = var.environment == "mgmt" ? 0 : 1
  provider = aws.us-east-1
  ...
  //TODO: see how this can be created with a module and accessed here instad of using someother modules
  alarm_actions = [module.acm_api_cert_alarms[0].sns_topic_arn]
