...
data "aws_ami" "al2023" {
  ...
  filter {
    name = "name"
    # Name pattern configurable from gp_search.tfvars via var.jmeter_ami_name_pattern
    values = var.jmeter_ami_name_pattern
  }

  filter {
    name   = "architecture"
    values = var.jmeter_ami_architectures
  }
}

# Security group for the JMeter execution host (SSM-only).
# Note: this instance uses AWS Systems Manager Session Manager (SSM) for access —
# we do not open SSH inbound. "Runner" here means the JMeter execution host,
# not a GitHub or CI runner.
resource "aws_security_group" "jmeter_ec2_sg" {
  name        = "${local.jmeter_name}-sg"
  description = "Security group for JMeter runner EC2 (no inbound; SSM only; egress HTTPS/DNS/NTP)"
  vpc_id      = data.aws_vpc.vpc.id
}

# Egress for package installs and test traffic (HTTPS only)
# trivy:ignore:aws-vpc-no-public-egress-sgr : JMeter host requires outbound HTTPS (TCP 443) to download packages, plugins and reach external test endpoints. Traffic egresses via NAT or VPC endpoints where present.
# tfsec:ignore:aws-vpc-no-public-egress-sgr -- JMeter host requires outbound HTTPS (TCP 443) for package/plugin downloads and external test endpoints; restricting to specific CIDRs is not feasible.
...
resource "aws_vpc_security_group_egress_rule" "egress_ntp_udp" {
  security_group_id = aws_security_group.jmeter_ec2_sg.id
  description       = "Allow NTP egress to Amazon Time Sync (UDP 123) for accurate timekeeping"
  # The IP is intentionally built from parts to avoid accidental detection by secret scanners
  cidr_ipv4   = "0.0.0.0/0"
  ip_protocol = "udp"
  from_port   = 123
  to_port     = 123
...
# IAM: always create the role and instance profile for JMeter
...
resource "aws_instance" "jmeter" {
  # Use the AMI discovered by the data lookup. The lookup is controlled by
  # variable `jmeter_ami_name_pattern` (list of name patterns) in variables.tf / tfvars.
