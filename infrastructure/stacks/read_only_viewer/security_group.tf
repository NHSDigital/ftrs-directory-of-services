...
resource "aws_vpc_security_group_ingress_rule" "frontend_lambda_allow_cloudfront" {
  ...
  security_group_id = aws_security_group.frontend_lambda_security_group[0].id
...
resource "aws_vpc_security_group_egress_rule" "frontend_lambda_allow_egress_to_internet" {
  count             = local.stack_enabled
