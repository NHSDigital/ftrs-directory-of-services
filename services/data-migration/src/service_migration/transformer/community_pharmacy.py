...
class CommunityPharmacyTransformer(ServiceTransformer):
    STATUS_ACTIVE = 1
    PHARMACY_TYPE_ID = 13
    ...
    """
    Transformer for Community Pharmacy services.

    Selection criteria:
    - The service type must be 'Pharmacy' (13)
    ...
    def is_service_supported(
        ...
        if not is_enabled(FeatureFlag.DATA_MIGRATION_PHARMACY_ENABLED):
            return (
                False,
                "Community Pharmacy service selection is disabled by feature flag",
            )

        if service.typeid != cls.PHARMACY_TYPE_ID:
            return False, "Service type is not Pharmacy (13)"
