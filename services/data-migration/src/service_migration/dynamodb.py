...
class ServiceTransactionBuilder:
    ...
    def __init__(
        ...
        self.migration_state = (
            ...
            if migration_state
            else MigrationState.create(service_id)
        )
        self.migration_state.validation_issues = validation_issues

        self.migration_state_table_name = self.format_migration_state_table_name(
            env=deps.config.env,
            workspace=deps.config.workspace,
        )
        self.logger = deps.logger
        self.record_id = service_id
    ...
    def _insert_organisation(self, organisation: Organisation | None) -> Self:
        ...
        if organisation is None:
            self.logger.log(ServiceMigrationLogBase.SM_PROC_011)
            return self
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_012,
    ...
    def _update_organisation(self, organisation: Organisation | None) -> Self:
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_018,
    ...
    def _insert_location(self, location: Location | None) -> Self:
        ...
        """
        if location is None:
            self.logger.log(ServiceMigrationLogBase.SM_PROC_013)
            return self
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_014,
    ...
    def _update_location(self, location: Location | None) -> Self:
        ...
        if not diff:
            self.logger.log(ServiceMigrationLogBase.SM_PROC_019)
            return self

        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_020,
            changes=diff.pretty().splitlines(),
            diff=diff.to_dict(view_override="text"),
        )
    ...
    def _insert_healthcare_service(
        ...
        """
        if healthcare_service is None:
            self.logger.log(ServiceMigrationLogBase.SM_PROC_015)
            return self
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_016,
    ...
    def _update_healthcare_service(
        ...
        if not diff:
            self.logger.log(ServiceMigrationLogBase.SM_PROC_021)
            return self

        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_022,
            changes=diff.pretty().splitlines(),
            diff=diff.to_dict(view_override="text"),
        )
    ...
    @classmethod
    def format_migration_state_table_name(
        cls,
        env: str,
        workspace: str | None = None,
    ) -> str:
        """Generate the migration state table name for the given environment.

        Args:
            env: The environment name (e.g., 'dev', 'prod').
            workspace: Optional workspace suffix for the table name.

        Returns:
            The formatted DynamoDB table name for storing migration state.
        """
        table_name = f"ftrs-dos-{env}-data-migration-state-table"
        if workspace:
    ...
    def _insert_state_record(self) -> Self:
        ...
        self.items.append(
            {
                "Put": {
                    "TableName": self.migration_state_table_name,
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_023,
    ...
    def _update_state_record(self) -> Self:
        ...
        self.logger.log(
            ServiceMigrationLogBase.SM_PROC_024,
            source_record_id=self.migration_state.source_record_id,
