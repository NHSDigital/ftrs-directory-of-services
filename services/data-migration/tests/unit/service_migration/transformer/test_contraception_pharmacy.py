...
def mock_feature_flags(mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch(
        "service_migration.transformer.contraception_pharmacy.is_enabled",
        return_value=True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            None,
        ),  # Valid alternating base ODS + CON
        (
            13,
            "FXX99CON",
            "Contraception: Test Service",
            False,
            "Service type is not a Contraception Pharmacy type (149)",
        ),  # Wrong type
        (
            149,
            None,
        ...
        (
            ...
            "Contraception: Test Service",
            False,
            "ODS code does not end with the required suffix (CON)",
        ),  # Missing CON suffix
        (
            149,
            "FXX99CON",
            "Test Service",
            False,
            "Service name does not start with the required prefix ('Contraception: ')",
        ),  # Wrong name prefix
        (
            149,
            "FXX99CON",
...
def test_is_service_supported(
    ...
    """Test that is_service_supported correctly validates Contraception Pharmacy services."""
    mock_legacy_service.typeid = service_type_id
    mock_legacy_service.odscode = ods_code
    mock_legacy_service.name = name

    is_supported, message = ContraceptionPharmacyTransformer.is_service_supported(
        mock_legacy_service
    )
...
def test_transform_creates_only_healthcare_service(
    ...
    transformer = ContraceptionPharmacyTransformer(MockLogger(), mock_metadata_cache)
    ...
    assert hs.type == HealthcareServiceType.ORAL_CONTRACEPTION_PRESCRIPTION_AND_SUPPLY
...
def test_resolve_parent_uses_existing_state_record(
    ...
    """When a state record already exists for the parent, return its org/loc IDs immediately."""
    mock_legacy_service.typeid = 149
    mock_legacy_service.odscode = "FXX99CON"
    mock_legacy_service.name = "Contraception: Test Service"
...
def test_resolve_parent_derives_correct_base_ods_code(
    ...
    """Test that the base ODS code is correctly derived by stripping the CON suffix."""
    mock_legacy_service.typeid = 149
    mock_legacy_service.odscode = "A1B2CCON"
    mock_legacy_service.name = "Contraception: Test Service"
