...
async def test_run_s3_restore(mocker: MockerFixture) -> None:
    mock_get_parameter = mocker.patch(
        ...
        return_value={
            ...
            "state-table": "s3://test-store/state-table.parquet",
        },
    )
    ...
    mock_read_parquet = mocker.patch(
        ...
        side_effect=[
            ...
            pd.DataFrame(data=[["state-table"]], columns=["data"]),
        ],
    )
    ...
    mock_read_parquet.assert_has_calls(
        [
            ...
            mocker.call(path="s3://test-store/state-table.parquet"),
        ]
    )
    ...
    mock_bulk_load_table.assert_has_calls(
        [
            ...
            mocker.call(
                "ftrs-dos-local-data-migration-state-table-fdos-000",
                ["state-table"],
            ),
        ]
    )
