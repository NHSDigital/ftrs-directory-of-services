...
def test_handle_event_dms_event(
    ...
    app.handle_full_sync_event = mocker.MagicMock()

    app.handle_event(mock_event)

    app.handle_dms_event.assert_called_once_with(
        DMSEvent(
            type="dms_event",
            record_id=123,
            table_name="test_table",
    ...
    app.handle_full_sync_event.assert_not_called()
    ...
    assert mock_logger.get_log("DM_ETL_000") == [
        {
            ...
            "msg": "Starting Data Migration ETL Pipeline",
            "reference": "DM_ETL_000",
        }
    ]
    assert mock_logger.was_logged("DM_ETL_999")


def test_handle_event_full_sync_event(
    mocker: MockerFixture,
    mock_logger: MockLogger,
    mock_config: DataMigrationConfig,
) -> None:
    ...
    app.handle_dms_event.assert_not_called()
    app.handle_full_sync_event.assert_called_once()

    assert mock_logger.get_log("DM_ETL_000") == [
        {
            "detail": {"event": {"type": "full_sync"}},
            "msg": "Starting Data Migration ETL Pipeline",
            "reference": "DM_ETL_000",
        }
    ]
