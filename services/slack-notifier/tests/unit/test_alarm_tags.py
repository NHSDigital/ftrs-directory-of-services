...
class TestGetAlarmTags:
    ...
    def test_get_alarm_tags_success(self, mock_boto_client: MagicMock) -> None:
        ...
        result = get_alarm_tags("arn:aws:cloudwatch:eu-west-2:00000000000:alarm:test")

        assert result == {"api_path": "/Organization", "service": "dos-search"}
        mock_cloudwatch.list_tags_for_resource.assert_called_once_with(
            ResourceARN="arn:aws:cloudwatch:eu-west-2:00000000000:alarm:test"
        )

    @patch("functions.alarm_tags.boto3.client")
    def test_get_alarm_tags_missing_tags(self, mock_boto_client: MagicMock) -> None:
        mock_cloudwatch = MagicMock()
        mock_boto_client.return_value = mock_cloudwatch
    ...
    @patch("functions.alarm_tags.boto3.client")
    def test_get_alarm_tags_partial_tags(self, mock_boto_client: MagicMock) -> None:
        mock_cloudwatch = MagicMock()
        mock_boto_client.return_value = mock_cloudwatch
        mock_cloudwatch.list_tags_for_resource.return_value = {
    ...
    @patch("functions.alarm_tags.boto3.client")
    def test_get_alarm_tags_client_error(self, mock_boto_client: MagicMock) -> None:
        mock_cloudwatch = MagicMock()
        mock_boto_client.return_value = mock_cloudwatch
    ...
    def test_get_alarm_tags_empty_arn(self) -> None:
        result = get_alarm_tags("")

        assert result == {"api_path": "N/A", "service": "Unknown"}

    def test_get_alarm_tags_none_arn(self) -> None:
