...
const OrganisationOverview: React.FC<{ organisation: Organisation }> = ({ organisation }) => {
  return (
    <Card className="nhsuk-u-padding-5">
      ...
      <SummaryList>
        ...
        <SummaryList.Row>
          ...
          <SummaryList.Value>{organisation.identifier_ODS_ODSCode}</SummaryList.Value>
        </SummaryList.Row>
        <SummaryList.Row>
        ...
        <SummaryList.Row>
          ...
          <SummaryList.Value>{organisation.active ? 'Yes' : 'No'}</SummaryList.Value>
        </SummaryList.Row>
        <SummaryList.Row>
          <SummaryList.Key>Telecom</SummaryList.Key>
          <SummaryList.Value>{organisation.telecom || <span className="nhsuk-caption">None Provided</span>}</SummaryList.Value>
        </SummaryList.Row>
        <SummaryList.Row>
          <SummaryList.Key>Created By</SummaryList.Key>
          <SummaryList.Value>{organisation.createdBy} <i className='nhsuk-u-font-size-16'>({organisation.createdDateTime})</i></SummaryList.Value>
        </SummaryList.Row>
        <SummaryList.Row>
          <SummaryList.Key>Last Modified By</SummaryList.Key>
          <SummaryList.Value>{organisation.modifiedBy} <i className='nhsuk-u-font-size-16'>({organisation.modifiedDateTime})</i></SummaryList.Value>
        </SummaryList.Row>
      </SummaryList>
    </Card>
  );
...
const OrganisationEndpoints: React.FC<{ endpoints: Endpoint[] }> = ({ endpoints }) => {
  const sortedEndpoints = useMemo(() => {
    return endpoints.sort((a, b) => a.payloadType === b.payloadType ? a.order - b.order : a.payloadType?.localeCompare(b.payloadType || '') || 0)
  }, [endpoints])
  ...
  const table = useReactTable({
    ...
    columns: [
      columnHelper.accessor('payloadType', {
        header: () => 'Payload Type',
        cell: info => <span style={{ maxWidth: 10, overflow: "scroll" }}>{humanReadableInteraction(info.getValue()) || 'Not Specified'}</span>,
      }),
        ...
        <Table.Body>
          {table.getRowModel().rows.map(row => (
            <Table.Row key={row.id}>
              ...
              <Table.Cell>
                <Link to="/organisations/$organisationID/endpoints/$endpointID" params={{ endpointID: row.id }}>View</Link>
              </Table.Cell>
            </Table.Row>
          ))}
        </Table.Body>
...
function RouteComponent() {
  ...
  const { data: organisation, isLoading, error } = useOrganisationQuery(organisationID);
  ...
  return (
    <>
      ...
      <Breadcrumb className="nhsuk-u-margin-bottom-3">
        <Breadcrumb.Back asElement={Link} to="/organisations">Back</Breadcrumb.Back>
        ...
        >
          {isLoading ? 'Loading' : error ? 'Error loading organisation' : organisation?.name || 'Organisation Details'}
        </Breadcrumb.Item>
      </Breadcrumb>
