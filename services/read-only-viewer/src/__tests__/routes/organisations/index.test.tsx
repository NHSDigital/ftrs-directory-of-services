...
describe("Organisations Index Route", () => {
  ...
  it("should render the Organisations page", async () => {
    ...
    await waitFor(() => expect(app.queryByText("Loading organisations...")).not.toBeInTheDocument());
    ...
    expect(firstRowCells[0].querySelector("a")).toHaveAttribute("href", "/organisations/e2f1d47c-a72b-431c-ad99-5e943d450f34");
    ...
    expect(secondRowCells[0].querySelector("a")).toHaveAttribute("href", "/organisations/763fdc39-1e9f-4e3d-bb69-9d1e398d0fdc");
  ...
  it("should handle errors gracefully", async () => {
    server.use(
      http.get('/api/organisations', () => {
        return HttpResponse.json(
          ...
          { status: 500, headers: { 'X-Correlation-ID': 'test-correlation-id' } }
        )
      }, {
        once: true
      })
    ...
    await waitFor(() => {
      expect(app.queryByText("Loading organisations...")).not.toBeInTheDocument();
    });

    expect(app.getByText("Something went wrong")).toBeInTheDocument();
    expect(app.getByText("There was an error while processing your request. Please try again later.")).toBeInTheDocument();

    expect(app.getByText("test-correlation-id")).toBeInTheDocument();
    expect(app.getByText("500")).toBeInTheDocument();
    expect(app.getByText("Failed to fetch organisations data")).toBeInTheDocument();
  })
});
