...
describe("PageBreadcrumbs", () => {
  ...
  it("renders breadcrumbs with correct links", async () => {
    ...
    const breadcrumbItems = container.querySelectorAll("li.nhsuk-breadcrumb__item");
    expect(breadcrumbItems.length).toBe(items.length);

    const homeLink = breadcrumbItems[0].querySelector("a.nhsuk-breadcrumb__link");
    expect(homeLink).toBeInTheDocument();
    expect(homeLink?.getAttribute("href")).toBe(items[0].to);
    expect(homeLink?.textContent).toBe(items[0].label);

    const pagesLink = breadcrumbItems[1].querySelector("a.nhsuk-breadcrumb__link");
    ...
    const pageByIdLink = breadcrumbItems[2].querySelector("a.nhsuk-breadcrumb__link");
