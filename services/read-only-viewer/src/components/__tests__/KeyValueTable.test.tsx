...
describe("KeyValueTable", () => {
  ...
  it("renders key-value pairs correctly", () => {
    ...
    const summaryListRows = container.querySelectorAll(
      "div.nhsuk-summary-list__row",
    );
    ...
    mockItems.forEach((item, idx) => {
      ...
      const keyElement = summaryListRow.querySelector(
        "dt.nhsuk-summary-list__key",
      );
      expect(keyElement).toBeInTheDocument();
      expect(keyElement?.textContent).toBe(item.key);

      const valueElement = summaryListRow.querySelector(
        "dd.nhsuk-summary-list__value",
      );
