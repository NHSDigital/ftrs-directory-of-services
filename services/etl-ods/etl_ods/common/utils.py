...
from etl_ods.common.auth import get_jwt_authenticator
...
def get_base_ods_terminology_api_url() -> str:
    ...
    if ods_url is None:
        ...
        utils_logger.log(OdsETLPipelineLogBase.ETL_UTILS_006, error_message=err_msg)
...
def _get_api_key_for_url(url: str) -> str:
    ...
    if env == "local":
        utils_logger.log(OdsETLPipelineLogBase.ETL_UTILS_005)
        return os.environ.get(
    ...
    **kwargs: dict,
    ...
    try:
        ...
        if response_correlation_id:
            utils_logger.append_keys(response_correlation_id=response_correlation_id)
        if response_request_id:
            utils_logger.append_keys(response_request_id=response_request_id)
