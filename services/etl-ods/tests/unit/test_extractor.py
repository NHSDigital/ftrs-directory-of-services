...
def test_processor_success(mocker: MockerFixture) -> None:
    ...
    mock_fetch = mocker.patch(
        "extractor.extractor.fetch_outdated_organisations",
        return_value=mock_organisations,
    )
    mock_send = mocker.patch("extractor.extractor._send_organisations_to_queue")
...
def test_processor_no_organisations(mocker: MockerFixture) -> None:
    ...
    mock_fetch = mocker.patch(
        "extractor.extractor.fetch_outdated_organisations", return_value=[]
    )
    mock_send = mocker.patch("extractor.extractor._send_organisations_to_queue")
...
def test_send_organisations_to_queue(mocker: MockerFixture) -> None:
    ...
    mocker.patch("extractor.extractor.get_correlation_id", return_value="corr-123")
    mocker.patch("extractor.extractor.get_request_id", return_value="req-456")
    mock_send_messages = mocker.patch("extractor.extractor.send_messages_to_queue")
...
def test_extractor_lambda_handler_success(mocker: MockerFixture) -> None:
    ...
    mock_processor = mocker.patch("extractor.extractor.processor")
