...
def test_consumer_lambda_handler_success(
    ...
    event = {
        "Records": [
            {"messageId": "1", "path": "/test1", "payload": {"key": "value1"}},
            {"messageId": "2", "path": "/test2", "payload": {"key": "value2"}},
        ]
    }
...
def test_consumer_lambda_handler_handle_missing_message_parameters(
    path: str, payload: dict, caplog: pytest.LogCaptureFixture
) -> None:
    event = {
        "Records": [
            {
    ...
    assert any(
        ...
        and "Message id: 1 is missing 'path' or 'payload' fields." in record.message
        for record in caplog.records
    )
    assert any(
        record.levelname == "ERROR"
