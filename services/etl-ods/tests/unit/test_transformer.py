...
def test_transformer_lambda_handler_success(mocker: MockerFixture) -> None:
    ...
    mock_send_messages = mocker.patch("transformer.transformer.send_messages_to_queue")
...
def test_transformer_lambda_handler_processing_failure(mocker: MockerFixture) -> None:
    ...
    mock_process = mocker.patch(
        "transformer.transformer._process_organisation", return_value=None
    )
    mock_send_messages = mocker.patch("transformer.transformer.send_messages_to_queue")
...
def test_process_organisation_success(mocker: MockerFixture) -> None:
    ...
    with patch("transformer.transformer.current_correlation_id") as mock_corr_id:
        with patch("transformer.transformer.current_request_id") as mock_req_id:
...
def test_process_organisation_uuid_not_found(mocker: MockerFixture) -> None:
    ...
    mocker.patch("transformer.transformer.fetch_organisation_uuid", return_value=None)
