...
with patch("ftrs_common.feature_flags.FeatureFlagsClient") as mock_client_class:
    ...
    from src.healthcare_service.handler import (
...
@pytest.fixture
def mock_error_util():
    with patch("src.healthcare_service.handler.error_util") as mock:
...
def mock_ftrs_service():
    with patch(
        "src.healthcare_service.handler.HealthcareServicesByOdsService"
    ) as mock_class:
...
@pytest.fixture
def mock_logger():
    with patch("src.healthcare_service.handler.logger") as mock:
...
def mock_feature_flags_client():
    with patch("src.healthcare_service.handler.FEATURE_FLAGS_CLIENT") as mock:
