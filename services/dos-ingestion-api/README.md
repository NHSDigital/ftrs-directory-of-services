...
This project provides testing for of the API that sends datat into the FtRS Directory of Services, hosted on the NHS Digital API Management Platform (APIM).
...
- `APIGEE_ENVIRONMENT` - The target Apigee environment (e.g., internal-dev)
- `API_KEY` - Valid API key for authentication

Copy `.env.example` and rename to `.env` and fill in the required values. Do not update the `env.example` file with the values as this is commited to version control.
