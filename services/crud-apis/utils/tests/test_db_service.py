...
def test_returns_correct_repository_for_healthcare_service():
...
def test_returns_correct_table_name_with_workspace():
    with patch("utils.db_service.get_env_variables") as mock_env_vars:
...
def test_raises_error_for_missing_env_variables():
    with patch("utils.db_service.get_env_variables") as mock_env_vars:
