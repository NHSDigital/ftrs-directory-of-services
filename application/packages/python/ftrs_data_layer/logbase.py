...
class OdsETLPipelineLogBase(LogBase):
    ...
    ETL_CONSUMER_007 = LogReference(
        level=INFO,
        message="Successfully sent PUT request to API for Organization {organization_id}. Response status code: {status_code}.",
    )
    ETL_UTILS_008 = LogReference(
        level=INFO,
        message="Running in automated test mode, using api key for mock from secret manager.",
    )
    ETL_UTILS_009 = LogReference(
        level=INFO,
        message="Running in automated test mode, sending request to mock API Gateway with x-api-key header",
    )
