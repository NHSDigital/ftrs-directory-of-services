...
@pytest.mark.parametrize(
    "resource", [{"resourceType": "DummyResource",
        "telecom": [
            {"system": "phone", "value": "123456"},
            {"system": "email", "value": "test@example.com"},
            {"system": "url", "value": "http://example.com"},
        ]}])
def test_check_telecom_types_are_valid(resource) -> None:
    model = DummyModel
    assert (
        FhirValidator._check_telecom_types_are_valid(resource, model) == resource
    )
@pytest.mark.parametrize(
    "resource, expected_type_error", [({
        "resourceType": "DummyResource",
        "telecom": [{"system": "web", "value": "123456"},]},
        "web"),
        ({"resourceType": "DummyResource",
        "telecom": [{"system": "invalid_type", "value": "123456"}]},
        "invalid_type")
        ])
def test_check_telecom_types_are_invalid(resource, expected_type_error) -> None:
    """
    ...
    assert f'Field \'telecom[0].system\' contains invalid telecom type: {expected_type_error}' in str(
        exc_info.value
    )
