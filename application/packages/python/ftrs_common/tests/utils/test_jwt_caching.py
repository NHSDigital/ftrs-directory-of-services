...
class TestJWTAuthenticatorCaching:
    ...
    def setup_rsa_keys(self) -> None:
        """Generate RSA key pair for testing."""
        if not self.RSA_PRIVATE_KEY or not self.RSA_PUBLIC_KEY:
            key = rsa.newkeys(2048)
            self.RSA_PRIVATE_KEY = key[1].save_pkcs1().decode("utf-8")
            self.RSA_PUBLIC_KEY = key[0].save_pkcs1().decode("utf-8")
    ...
    def mock_environment(self) -> None:
        ...
        env_vars = {
            ...
            "LOCAL_PRIVATE_KEY": self.RSA_PRIVATE_KEY,
            "LOCAL_KID": "test-kid",
    ...
    @pytest.fixture
    def jwt_authenticator(self, mock_environment: Callable) -> JWTAuthenticator:
        """Create a JWT authenticator instance for testing."""
        return JWTAuthenticator(environment="local")

    @patch("ftrs_common.utils.jwt_auth.requests.post")
    def test_bearer_token_caching_first_request(
        ...
        assert jwt_authenticator.cached_token is None
        assert jwt_authenticator.token_expires_at is None

        # First request should generate new token
        token = jwt_authenticator.get_bearer_token()

        assert token == "test-bearer-token"
        assert jwt_authenticator.cached_token == "test-bearer-token"
        assert jwt_authenticator.token_expires_at is not None
    ...
    @patch("ftrs_common.utils.jwt_auth.requests.post")
    @patch("ftrs_common.utils.jwt_auth.time")
    def test_bearer_token_cache_expiry(
        ...
        assert mock_post.call_count == excepted_call_count
    ...
    def test_bearer_token_caching_after_error_recovery(
        ...
        assert jwt_authenticator.cached_token == "recovery-token"

    def test_cache_isolation_between_instances(
        self, mock_environment: Callable
    ) -> None:
        ...
        auth1.cached_token = "token-1"
        auth1.token_expires_at = time() + 300

        # Second instance should have empty cache
        assert auth2.cached_token is None
        assert auth2.token_expires_at is None

        # Set cache on second instance
        auth2.cached_token = "token-2"
        auth2.token_expires_at = time() + 300

        # Both instances should maintain their own caches
        assert auth1.cached_token == "token-1"
        assert auth2.cached_token == "token-2"
